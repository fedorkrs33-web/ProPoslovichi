...
export default function HomePage() {
  ...
  const { proverbs, loading, refetch } = useProverbs() // ← все данные отсюда
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/proverbs', {
        ...
        body: JSON.stringify({
          ...formData,
          authorId: session?.user?.id, // если есть авторизация
        }),
      })
  
      if (res.ok) {
        const newProverb = await res.json()
        console.log('Добавлено:', newProverb)
  
        // ✅ Обновляем список через refetch
        refetch()
        ...
        alert('Ошибка при добавлении пословицы')
      }
    } catch (error) {
